...
# Slide 1 already has its VO (archival FDR speech clip, not Kokoro) — only
# slides 2, 3, and the end card need generation here.
LINES = [
    ("02", "The first draft of the speech stated 'a date which will live in world history.' FDR crossed it out himself, in pencil."),
    ("03", "He changed 'simultaneously' to 'suddenly' and deleted the closing phrase 'without warning.'"),
    ("04", "Comment FRONT for the free WWII document."),
]
...
def main() -> None:
    print("=== Kokoro TTS — day-of-infamy-draft voiceovers ===\n")
    ...
    for num, text in LINES:
        ...
        mp3_path = AUDIO_DIR / f"day-of-infamy-draft-vo-{num}.mp3"
    ...
    for num, path, sample_dur in generated:
        ...
        print(f"  day-of-infamy-draft-vo-{num}.mp3  {dur:.3f}s  ->  durationInFrames: {frames}  ({dur:.3f} + {PAD}s pad x {FPS}fps)")
