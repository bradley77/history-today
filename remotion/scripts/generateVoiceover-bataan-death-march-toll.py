...
LINES = [
    ("vo-01", "The march wasn't the deadliest part."),
    ("vo-02", "Five hundred Americans died walking sixty-five miles."),
    ("vo-03", "Up to twenty thousand prisoners died at Camp O'Donnell."),
    ("vo-04", "The general in charge was executed for it."),
    ("vo-05", "Comment FRONT for the free PDF."),
]
...
def main() -> None:
    print("=== Kokoro TTS — bataan-death-march-toll voiceovers ===\n")
    ...
    for num, text in LINES:
        ...
        mp3_path = AUDIO_DIR / f"bataan-death-march-toll-{num}.mp3"
        ...
        print(f"  {num}: {duration_s:.3f}s  ({size_kb:.0f} KB)  {out_path.name}")

    print(f"\nDurations (for durationInSeconds = actual + 0.4s pad):")
    for (num, _), dur in zip(LINES, durations):
        print(f"  {num}: {dur:.3f}s  ->  durationInSeconds: {dur + 0.4:.3f}")
