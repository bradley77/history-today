...
const PY_SCRIPT = path.join(__dirname, 'generateVoiceover-eisenhower-photographed-evidence.py');
const AUDIO_DIR = path.join(__dirname, '..', 'public', 'audio');

console.log('Kokoro TTS — eisenhower-photographed-evidence voiceovers\n');
...
for (const num of slides) {
  const mp3 = path.join(AUDIO_DIR, `eisenhower-photographed-evidence-vo-${num}.mp3`);
