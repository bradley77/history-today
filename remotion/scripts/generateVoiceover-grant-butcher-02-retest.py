"""Regenerate Slide 2 for grant-butcher as a test-a variant, using the same
ChatterboxTurboTTS settings as grant-butcher-vo-01-test-a.mp3 (Turbo
defaults: temperature=0.8, top_k=1000, top_p=0.95, repetition_penalty=1.2)
and the same despotism-ref.wav reference clip, for voice consistency with
slides 1/3/4. Writes a test-a variant only -- does not touch the locked
grant-butcher-vo-02.mp3 or any composition."""
...
STEM = "grant-butcher-vo-02-test-a"
TEXT = (
    "But at the Wilderness, Spotsylvania, and North Anna, he kept moving south "
    "around Lee's flank instead of storming every position in his path."
)
