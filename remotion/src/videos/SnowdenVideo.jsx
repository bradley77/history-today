...
import { slides } from '../data/snowden';
...
const getKenBurnsTransform = (motionType, localFrame, duration) => {
  switch (motionType) {
    ...
    case 'zoomInAggressive': {
      const scale = interpolate(localFrame, [0, duration], [1.0, 1.4], { extrapolateRight: 'clamp' });
      return `scale(${scale})`;
    }
    case 'zoomIn':
    default: {
...
export default function SnowdenVideo() {
  ...
  // Hard cut flash between slides (4 frames) — skip on slide 0 so video opens clean
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: '#000' }}>
      <Audio src={staticFile('audio/snowden-voiceover.mp3')} />
      <Audio src={staticFile('audio/snowden_music.mp3')} volume={0.15} startFrom={0} loop={true} />
      ...
      >
        <Img
          ...
          style={{
            ...
            transformOrigin: activeSlide.transformOrigin || 'center center',
            transform,
          }}
        />
      </div>
